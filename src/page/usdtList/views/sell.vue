          ...
          <div class="list-content-info">
            <div class="list-content-info-left">
              <p>
                ...
                <span>{{ item.price }}CNY/USDT</span>
              </p>
              <p>
                <span>{{ $t("dialog.coincharging.text7") }}</span>
                <span>{{ item.usdtNum }}USDT</span>
              </p>
              <p>
                <span>{{ $t("legalTransaction.sellTime") }}</span>
                <span> {{ item.withdrawTime }}</span>
              </p>
              <p>
                <span>{{ $t("legalTransaction.text6") }}</span>{{ item.remark }}
              </p>
            </div>
            <div class="list-content-info-right">
              {{
                item.status === "NOTPASS"
                  ? $t("legalTransaction.status22")
                  : item.status === "PENDING"
                  ? $t("legalTransaction.status11")
                  : item.status === "PROCESSING"
                  ? $t("legalTransaction.status33")
                  : item.status === "SUCCESSFUL"
                  ? $t("legalTransaction.status44")
                  : item.status === "FAILED"
                  ? $t("legalTransaction.status55")
                  : ""
              }}
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...mapActions({
      otcWithdraw: "user/otcWithdraw" //充值列表
    }),
    change({ value, key }) {
    ...
    getList() {
      ...
      this.otcWithdraw(params).then(res => {
        console.log(res);
        console.log(1111);

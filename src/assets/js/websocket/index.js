...
export class WebSocketClient extends EventEmitter {
    ...
    constructor(address = "ws://localhost:8080", {
            ...
            autoconnect = true,
            reconnect = true,
            heartbeat_inteval = 30 * 1000, // 心跳间隔
         ...
         * @return {Undefined}
         */
        // connect(address = this.address, options = this.options) {
        //     if (this.socket)
        //         return
        ...
        var _visibleLister = () => {
            if (document) {
                if (document.visibilityState == "hidden") {
                    ...
                    if ((visibleTime - this.hiddenTime) > 90 * 1000) {
                        console.log("超过90秒，灭屏重显")
                            // dbgLog("灭屏重显。。超过30秒")
                        this.close(3000)
                    } else {
                        this._resetTimer()
    ...
    _connect() {
        ...
        this.socket = new WebSocketBrowser(this.address, this.options)
